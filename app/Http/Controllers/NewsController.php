...
class NewsController extends Controller
{
    ...
    public function create()
    {
        ...
        if (Auth::user()->role->id == '3' && View::exists('admin.news.create')) {
            ...
            return view('superadmin.news.create', compact('languages'));
        } else if (Auth::user()->role->id == '1' && View::exists('serveradmin.news.create')) {
            return view('serveradmin.news.create', compact('languages'));
        } else {
    ...
    public function edit($id)
    {
        ...
        if ($newses) {
            if (Auth::user()->role->id == '3') {
                return view('admin.news.edit', compact('newses','languages'));
            } else if (Auth::user()->role->id == '2') {
                return view('superadmin.news.edit', compact('newses','languages'));
            } else if (Auth::user()->role->id == '1') {
                return view('serveradmin.news.edit', compact('newses','languages'));
            }
        } else {
            if (Auth::user()->role->id == '3') {
