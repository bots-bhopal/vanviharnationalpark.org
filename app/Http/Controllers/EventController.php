...
class EventController extends Controller
{
    ...
    public function create()
    {
        ...
        if (Auth::user()->role->id == '3' && View::exists('admin.event.create')) {
            return view('admin.event.create', compact('languages'));
        } else if (Auth::user()->role->id == '2' && View::exists('superadmin.event.create')) {
            return view('superadmin.event.create', compact('languages'));
        } else if (Auth::user()->role->id == '1' && View::exists('serveradmin.event.create')) {
            return view('serveradmin.event.create', compact('languages'));
        } else {
    ...
    public function edit($id)
    {
        ...
        if ($events) {
            if (Auth::user()->role->id == '3') {
                return view('admin.event.edit', compact('events','languages'));
            } else if (Auth::user()->role->id == '2') {
                return view('superadmin.event.edit', compact('events','languages'));
            } else if (Auth::user()->role->id == '1') {
                return view('serveradmin.event.edit', compact('events','languages'));
            }
        } else {
            if (Auth::user()->role->id == '3') {
                return redirect()->route('admin.events.index')->with('error', __('message.event-error'));
            } else if (Auth::user()->role->id == '2') {
                return redirect()->route('superadmin.events.index')->with('error', __('message.event-error'));
            } else if (Auth::user()->role->id == '1') {
                return redirect()->route('serveradmin.events.index')->with('error', __('message.event-error'));
            }
        }
    }
