            ...
            <div class="row">
                <div class="col-lg-8 col-md-12">
                    <div class="card card-primary card-outline">
                        ...
                        <form action="{{ route('superadmin.events.update', $events->id) }}" method="POST"
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <select name="languages" class="form-control @error('languages') is-invalid @enderror">
                                        ...
                                        @foreach($languages as $language)
                                            <option @if($events->language == $language->id) selected @endif value="{{$language->id}}">{{$language->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('languages')
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('title') is-invalid @enderror"
                                            id="title" name="title" value="{{ $events->title }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{$events->slug}}" placeholder="Enter News Slug" disabled>
                                        {{-- <span class="input-group-append">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="time" class="form-control @error('time') is-invalid @enderror" id="time"
                                        name="time" value="{{ date('H:i', strtotime($events->time)) }}">
                                    @error('time')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('place') is-invalid @enderror"
                                            id="place" name="place" value="{{ $events->place }}"
                                            placeholder="Enter Event Place">
                                        <span class="input-group-append">
                                            <button type="button" class="btn btn-primary far fa-clipboard copy-button"
                                                data-clipboard-target="#place" data-toggle="tooltip" data-placement="top"
                                                title="Copy to clipboard"
                                                style="border-top-right-radius: .25rem; border-bottom-right-radius: .25rem;"></button>
                                        </span>
                                        @error('place')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="ckeditor form-control @error('description') is-invalid @enderror" rows="5" id="description"
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="custom-file">
                                        <input type="file"
                                            class="custom-file-input border @error('image') is-invalid @enderror"
                                            name="image">
                                        <label class="custom-file-label"
                                            for="customFile">{{ __('panel.event-image-file') }}</label>
                                        @error('image')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                            ...
                            <div class="card-footer">
                                <button type="submit" class="btn btn-primary"><i class="nav-icon fa fa-pencil-alt"
                                        style="margin-right: 5px;"></i>{{ __('panel.event-btn-update') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card card-outline card-primary">
                        ...
                        <div class="card-body">
                            ...
                            @if (!empty($events['image']) && file_exists($path))
                                <img src="{{ asset('public/storage/event_image/' . $events->image) }}" width="100%"
                        ...
                        @if (!empty($events['image']) && file_exists($path))
                            <div class="card-footer d-flex">
                                <button type="button" class="btn btn-danger btn-sm"
                                    onclick="deleteImage({{ $events->id }})">Delete Image<i
                                        class="fas fa-trash-alt pl-1"></i></button>

                                <form id="delete-image-form-{{ $events->id }}"
                                    action="{{ route('superadmin.events.destroy_image', $events->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                    ...
                                    <div class="col-lg-3 col-md-4 col-sm-12">
                                        <div class="card mb-2 ">
                                            ...
                                            @if (!empty($image['image']) && file_exists($path))
                                                <div class="card-footer d-flex">
                                                    <button type="button" class="btn btn-danger btn-sm"
                                                        onclick="deleteImages({{ $image->id }})">Delete Image<i
                                                            class="fas fa-trash-alt pl-1"></i></button>

                                                    <form id="delete-images-form-{{ $image->id }}"
                                                        action="{{ route('superadmin.events.destroy_images', $image->id) }}"
                                ...
                                @empty
                                    <div class="col-lg-12">
                                        <h5 class="font-weight-bold text-center">
                                            {{ __('panel.news-image-not-available') }}</h5>
                                    </div>
                                @endforelse
                            </div>
                        </div>
