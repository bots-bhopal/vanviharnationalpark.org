                        ...
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                ...
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip"
                                    data-placement="top" title="{{ __('panel.event-tooltip') }}"
                                    style="font-size: 14px; font-weight: 500;">{{ $events->count() }}</span>
                            </h3>

                            <a class="btn btn-sm btn-primary ml-auto" href="{{ route('serveradmin.events.create') }}"><i
                                    class="nav-icon fas fa-plus-circle"
                                    style="margin-right: 5px;"></i>{{ __('panel.event-btn-add') }}</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if (!empty($event['image']) && file_exists($path))
                                                    <img src="{{ asset('public/storage/event_image/' . $event->image) }}"
                                                        width="150" height="100" class="img-responsive rounded" alt="image">
                                                @else
                                            ...
                                            <td>
                                                <input data-id="{{ $event->id }}" class="toggle-class" type="checkbox"
                                                    ...
                                                    data-toggle="toggle" data-style="slow" data-on="Approved"
                                                    data-off="Pending" {{ $event->status ? '' : 'checked' }}>
                                            </td>
                                            <td class="text-center w-25">
                                                <a href="{{ route('serveradmin.events.edit', $event->id) }}"
                                                    class="btn btn-sm btn-info mt-1"><i class="nav-icon fas fa-edit"></i>
                                                    Edit</a>

                                                <button type="button" class="btn btn-sm btn-danger mt-1"
                                                    onclick="deleteEvent({{ $event->id }})"><i
                                                        class="nav-icon fas fa-trash-alt"></i> Delete</button>
                                                <form id="delete-form-{{ $event->id }}"
                                                    action="{{ route('serveradmin.events.destroy', $event->id) }}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        data-dismiss="modal">{{ __('panel.event-modal-btn-cancel') }}</button>
                    <button type="button" class="btn btn-danger"
                        id="delEvent">{{ __('panel.event-modal-btn-yes') }}</button>
                </div>
            </div>
        </div>
    </div>
