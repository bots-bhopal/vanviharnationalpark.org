                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                ...
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip"
                                    data-placement="top" title="Total Number of Pending Document's"
                                    style="font-size: 14px; font-weight: 500;">{{ $documents->count() }}</span>
                            </h3>
                            ...
                            <table id="tblDocs" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ __('panel.document-description') }}</th>
                                ...
                                @foreach ($documents as $key => $document)
                                    <tr>
                                        ...
                                        <td width="15%" class="text-justify">{!! $document->description !!}</td>
                                            ...
                                            @if ($document->file_extension)
                                                @if ($document->file_extension == 'doc' || $document->file_extension == 'docx')
                                                    <img src="{{ asset('public/assets/images/doc-icon/word.png') }}"
                                                        width="24" height="24" class="img-responsive rounded"
                                                        alt="doc-image">
                                                @endif

                                                @if ($document->file_extension == 'pdf')
                                                    <img src="{{ asset('public/assets/images/doc-icon/pdf.png') }}"
                                                        width="28" height="28" class="img-responsive rounded"
                                                        alt="doc-image">
                                                @endif
                                            @endif
                                        </td>
                                        <td width="10%" class="text-center">
                                            @if ($document->file_extension == 'doc' || $document->file_extension == 'docx')
                                                <span class="badge badge-primary"
                                                    style="font-size: 14px;">{{ HumanReadable::bytesToHuman($document->file_size) }}</span>
                                            @endif
                                        ...
                                        <td class="text-center">
                                            <input data-id="{{ $document->id }}" class="toggle-class" type="checkbox"
                                                data-onstyle="success" data-offstyle="danger" data-size="small"
                                                data-toggle="toggle" data-style="slow" data-on="Approved" data-off="Pending"
                                                {{ $document->status ? '' : 'checked' }}>
                                        </td>
                                        <td class="text-center">
                                            <a href="{{ route('serveradmin.document.edit', $document->id) }}"
                                                class="btn btn-sm btn-primary"><i class="far fa-edit"
                                                    style="margin-right: 5px;"></i>Edit</a>

                                            <button type="button" class="btn btn-sm btn-danger"
                                                onclick="deleteTender({{ $document->id }})"><i class="fa fa-trash-alt"
                                                    style="margin-right: 5px;"></i>Delete</button>
                                            <form id="delete-form-{{ $document->id }}"
                                                action="{{ route('serveradmin.document.destroy', $document->id) }}"
                                                method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>

                                            <a href="{{ route('vvnp.docs.download', $document->original_filename) }}"
                                                class="btn btn-sm btn-success"><i class="fa fa-download"
                                                    style="margin-right: 4px;"></i>Download</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </table>
