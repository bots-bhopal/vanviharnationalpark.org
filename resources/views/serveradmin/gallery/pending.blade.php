                                ...
                                <div class="card-header d-flex">
                                    <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                        ...
                                        <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip"
                                            data-placement="top" title="Total Number of Pending Images"
                                            style="font-size: 14px; font-weight: 500;">{{ $images->count() }}</span>
                                    </h3>
                                        ...
                                        <tbody>
                                            @forelse ($images as $key=>$image)
                                                <tr>
                                                    ...
                                                    <td class="text-center">{{ ucwords($image->original_filename) }}
                                                    </td>
                                                    <td class="text-center">
                                                        <div class="image-box">
                                                            <a href="{{ asset('public/storage/gallery/' . $image->filename) }}"
                                                                data-toggle="lightbox"
                                                                data-title="{{ $image->original_name }}"
                                                                data-gallery="gallery">
                                                                <img class="img-responsive pad rounded"
                                                                    src="{{ asset('public/storage/gallery/' . $image->filename) }}"
                                                                    width="150" height="100" alt="Gallery Image">
                                                            </a>
                                                        </div>
                                                    </td>
                                                    <td class="text-center">
                                                        <span
                                                            class="badge badge-info">{{ HumanReadable::bytesToHuman($image->file_size) }}</span>
                                                    </td>
                                                    <td class="text-center">
                                                        <input data-id="{{ $image->id }}" class="toggle-class"
                                                            type="checkbox" data-onstyle="success" data-offstyle="danger"
                                                            data-size="small" data-toggle="toggle" data-style="slow"
                                                            data-on="Approved" data-off="Pending"
                                                            {{ $image->status ? '' : 'checked' }}>
                                                    </td>
                                                    <td class="text-center">
                                                        <button type="button" class="btn btn-danger btn-sm"
                                                            onclick="deleteImage({{ $image->id }})">{{ __('panel.btn-delete') }}
                                                            <i class="fas fa-trash-alt pl-1"></i></button>
                                                        <form id="delete-form-{{ $image->id }}"
                                                            action="{{ route('serveradmin.images.destroy', $image->id) }}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        data-dismiss="modal">{{ __('panel.gallery-modal-btn-cancel') }}</button>
                    <button type="button" class="btn btn-danger"
                        id="delImage">{{ __('panel.gallery-modal-btn-yes') }}</button>
                </div>
            </div>
        </div>
    </div>
