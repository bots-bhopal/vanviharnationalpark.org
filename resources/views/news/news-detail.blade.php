                        ...
                        <div class="col-md-12 blog-post-hr image-block">
                            <div class="blog-post mb-30">
                                    ...
                                    <div class="images image-box">
                                        <figure class="image">
                                            ...
                                            @if (!empty($newsDetail['image']) && file_exists($path))
                                                <img src="{{ asset('public/storage/news_image/' . $newsDetail->image) }}"
                                                    class="lazy-image owl-lazy" alt="">
                                                <a href="{{ asset('public/storage/news_image/' . $newsDetail->image) }}"
                                                    class="lightbox-image icon" data-fancybox="image"><span
                                                        class="fa fa-search"></span></a>
                                            @else
                                                <img src="{{ asset('public/assets/images/' . $newsDetail->image) }}"
                                ...
                                <div class="post-header">
                                    <h2 class="post-title">{{ $newsDetail->title }}</h2>
                                </div>

                                <div class="post-meta">
                                    <span class="badge badge-pill badge-mix post-date">Posted On: {{ date('d-M-Y', strtotime($newsDetail->date)) }}</span>
                                </div>
                                        ...
                                        @if ($newsDetail)
                                            @foreach ($newsDetail->images as $image)
                                                <div class="item image-box">
                                                    <figure class="image">
                                                        <img
                                                            src="{{ asset('public/storage/news_images/' . $image->image) }}">
                                                        <div class="overlay-box">
                                                            <ul class="option-box">
                                                                <li><a href="{{ asset('public/storage/news_images/' . $image->image) }}"
