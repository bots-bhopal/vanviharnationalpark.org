                        ...
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                ...
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip"
                                    data-placement="top" title="Total Number of Pending Newses"
                                    style="font-size: 14px; font-weight: 500;">{{ $pendingNews->count() }}</span>
                            </h3>
                                    ...
                                    @foreach ($pendingNews as $key => $news)
                                        <tr>
                                            ...
                                            <td class="text-center w-25">
                                                <a href="{{ route('admin.newses.edit', $news->id) }}"
                                                    class="btn btn-sm btn-info mt-1"><i class="nav-icon fas fa-edit"
                                                        style="margin-right: 5px;"></i>{{ __('panel.news-btn-edit') }}</a>

                                                <button type="button" class="btn btn-sm btn-danger mt-1"
                                                    onclick="deletePost({{ $news->id }})"><i
                                                        class="nav-icon fas fa-trash-alt"
                                                        style="margin-right: 5px;"></i>{{ __('panel.news-btn-delete') }}</button>
                                                <form id="delete-form-{{ $news->id }}"
                                                    action="{{ route('admin.newses.destroy', $news->id) }}" method="POST"
                                                    style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        data-dismiss="modal">{{ __('panel.news-modal-btn-cancel') }}</button>
                    <button type="button" class="btn btn-danger"
                        id="delPost">{{ __('panel.news-modal-btn-yes') }}</button>
                </div>
            </div>
        </div>
    </div>
