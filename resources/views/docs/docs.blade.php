    ...
    <div id="testimonial-section" class="faq padding pt-xs-40">
        ...
        <div class="pagination-nav text-left mtb-xs-30">
            @if ($docs->lastPage() > 1)
                <ul>
                    @if ($docs->onFirstPage())
                        <li><a class="{{ $docs->currentPage() == 1 ? 'disabled' : '' }}"><i
                    ...
                    @for ($i = 1; $i <= $docs->lastPage(); $i++)
                        <li>
                            <a class="{{ $docs->currentPage() == $i ? ' active' : '' }}"
                                href="{{ $docs->url($i) }}">{{ $i }}</a>
                        </li>
                    @endfor

                    @if ($tenders->hasMorePages())
                        <li><a class="{{ $docs->currentPage() == $docs->lastPage() ? ' disabled' : '' }}"
                                href="{{ $docs->nextPageUrl() }}"><i class="fa fa-angle-right"></i></a></li>
                    @else
